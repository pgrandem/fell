...
/// tRate() - 22.06.2017
/// test interactions rates
void tRate()
{
  /// function header
  /// --------------------------------------------------------------------------
  cout << endl; cout << "tRate() - 22.06.2017" << endl;
  for( int i=0; i<80; ++i) { cout << "*"; } cout << endl;
  cout << "test interaction rates" << endl;
  ...
  RGas* gas = new RGas("repTest", pre, tem);
  gas->setCompound(mov, mfv);
  /// dump the gas
  cout << endl << "rep gas mixing" << endl;
  gas->dump(cout, "pkr261");
  
  /// define machine 
  RMachine* ele = new RMachine("elena", 75.*RUnits::um, 3.*RUnits::m, 
                               30.4055*RUnits::m);
  double eaa = ele->acceptanceAngle();
  cout << endl;
  cout << "angle = " << eaa << endl;
  
  /// define interactions
  RInteraction_FlyerGas* ifg = new RInteraction_FlyerGas("npi", fly, gas);
  
  /*cout << "ics = " << ifg->ion_csGas() << " | ";
  cout << "scs = " << ifg->sca_csGas() << " - ";
  cout << "lcs = " << ifg->lasl_csGas(eaa) << "; ";
  cout << "bcs = " << ifg->ebu_csGas(eaa) << "; ";
  cout << endl;*/
  
  vector<double> vec=ifg->ebu_csMol(eaa);
  ...
  /// end of function
  /// --------------------------------------------------------------------------
  cout << endl; cout << "tRate() function end " << endl;
  for( int i=0; i<80; ++i) { cout << "*"; } cout << endl;
}
...
void molIonCSeval()
{
  ...
  /// do stuff 
  cout << (RParticleList::mCO2())->csIonisation(500.*RUnits::keV) << "m2" << endl;
