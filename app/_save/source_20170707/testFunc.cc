...
void ecplots()
{
  ...
  /// do the stuff
  /// --------------------------------------------------------------------------
  int const np(51);         /// number of points
...
TGraph* cg(int np, int cns, double xsa[], double ysa[], string const& ymodel)
{
  ...
  /// cooling model : linear at deceleration, cooling curve at cooling
    ...
    TGraph* fcg = ecool("35MeV_c"); /// first cooling graph (betaCool sim)
    TGraph* scg = ecool("100keV");  /// second cooling graph (betaCool sim)
    for( int i=1; i<np; ++i ) { /// loop over sampling points
      /// x array
      gxa[i] = xstep*i + xsa[0];
      /// y array
      for( int j=0; j<cns-1; ++j ) {  /// loop over cycle step points
        if( gxa[i]>xsa[j] && gxa[i]<=xsa[j+1] && j==3 ) { /// 1st cooling
          //gya[i]=ecoolEmit(fcg, gxa[i], ysa[j], ysa[j+1]);
          gya[i]=RMath::y2x(xsa[j],xsa[j+1],gxa[i],ysa[j],ysa[j+1],"linear");
        }
        else if( gxa[i]>xsa[j] && gxa[i]<=xsa[j+1] && j==7 ) { /// second cooling
          //gya[i]=ecoolEmit(ecg, gxa[i], ysa[j], ysa[j+1]);
          gya[i]=RMath::y2x(xsa[j],xsa[j+1],gxa[i],ysa[j],ysa[j+1],"linear");
        }
...
void ecoolplot()
{
  ...
  eco->Draw("ALP");
  gPad->Print("../results/20170706/eco.pdf", "pdf");
  TGraph* ect = ecool("100keV");
  ect->SetMarkerStyle(4);
  ect->SetMarkerSize(0.4);
  ect->Draw("ALP");
  gPad->Print("../results/20170706/ect.pdf", "pdf");
  delete eco; eco=0;
  delete ect; ect=0;
}
...
/// scale curve to input param zi and zf, then evaluate at x
double ecoolEmit(TGraph* const& tg, double const& x, 
                 double const& zi, double const& zf)
{
...
{
  /// directory path
  const string path("~/private/programming/root/repShared/repData/coolingData/");
  //const string path("afs/cern.ch/user/p/pgrandem/private/programming/root/repShared/repData/coolingData/");
  //const string path("~/setve");
