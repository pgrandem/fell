...
void elenaCycle()
{
  ...
  const int fc_nsp(27);     /// function const : number of sampling points
  ...
  /// emittance steps
  const double emx_inj(0.5*RUnits::um);   double emy_inj(0.3*RUnits::um); 
  const double emx_fdb(0.5*RUnits::um);   double emy_fdb(0.3*RUnits::um);
  const double emx_fde(1.7*RUnits::um);   double emy_fde(1.1*RUnits::um);
  const double emx_feb(1.7*RUnits::um);   double emy_feb(1.1*RUnits::um); 
  const double emx_fee(0.45*RUnits::um);  double emy_fee(0.42*RUnits::um); 
  const double emx_sdb(0.45*RUnits::um);  double emy_sdb(0.42*RUnits::um); 
  const double emx_sde(2.2*RUnits::um);   double emy_sde(2.5*RUnits::um); 
  const double emx_seb(2.2*RUnits::um);   double emy_seb(2.5*RUnits::um); 
  const double emx_see(0.3*RUnits::um);   double emy_see(0.2*RUnits::um); 
  const double emx_ext(1.2*RUnits::um);   double emy_ext(0.75*RUnits::um); 
  /// boolean sequence checker : initialised at false = never been in sequence
  bool bsc_inj(false);    /// injection
  bool bsc_fdb(false);    /// first deceleration begin
  bool bsc_fde(false);    /// first deceleration end
  bool bsc_feb(false);    /// first electron cooling start
  bool bsc_fee(false);    /// first electron cooling end
  bool bsc_sdb(false);    /// second deceleration begin
  bool bsc_sde(false);    /// second deceleration end
  bool bsc_seb(false);    /// second electron cooling start
  bool bsc_see(false);    /// second electron cooling end
  bool bsc_ext(false);    /// extraction (rebunched);
  
  /// beam/machine parameters
  const double be_ncp=2e7;  /// beam - number of circulationg pbar
  
  /// arrays
  double tc[fc_nsp];  /// time cycle - array of sampling point
  double ke[fc_nsp];  /// kinetic energy - array of sampling point
  double emx[fc_nsp]; /// horizontal physical emittance (um, sigma2/TwissB)
  double emy[fc_nsp]; /// vertical physical emittance (um, sigma2/TwissB)
  
  
  /// time cycle
  /// ----------
  double tc_tsv((tc_ext-tc_inj)/(fc_nsp-1));  /// time cycle - time step value
  for( int i=0; i<fc_nsp; ++i ) { tc[i]=tc_tsv * i + tc_inj; }
  ...
  emx[0]=emx_inj;
  emy[0]=emy_inj;
  for( int i=1; i<fc_nsp; ++i ) { /// main loop
    /// inj -> fdb : between injection and first deceleration
    if( tc[i]>tc_inj && tc[i]<=tc_fdb ) { 
      ke[i]  = stepper(tc_inj,tc_fdb,tc_tsv,ke_inj,ke_fdb,ke[i-1], bsc_inj);
      emx[i] = stepper(tc_inj,tc_fdb,tc_tsv,emx_inj,emx_fdb,emx[i-1], bsc_inj);
      emy[i] = stepper(tc_inj,tc_fdb,tc_tsv,emy_inj,emy_fdb,emy[i-1], bsc_inj);
    }
    /// fdb -> fde : first deceleration 
    else if( tc[i]>tc_fdb && tc[i]<=tc_fde )   { 
      ke[i]  = stepper(tc_fdb,tc_fde,tc_tsv, ke_fdb,ke_fde,ke[i-1], bsc_fdb);
      emx[i] = stepper(tc_fdb,tc_fde,tc_tsv, emx_fdb,emx_fde,emx[i-1], bsc_fdb);
      emy[i] = stepper(tc_fdb,tc_fde,tc_tsv, emy_fdb,emy_fde,emy[i-1], bsc_fdb);
    } 
    /// fde -> feb : between 1st deceleration and 1st electron cooling
    else if( tc[i]>tc_fde &&  tc[i]<=tc_feb )   { 
      ke[i]  = stepper(tc_fde,tc_feb,tc_tsv, ke_fde,ke_feb,ke[i-1], bsc_fde);
      emx[i] = stepper(tc_fde,tc_feb,tc_tsv, emx_fde,emx_feb,emx[i-1], bsc_fde);
      emy[i] = stepper(tc_fde,tc_feb,tc_tsv, emy_fde,emy_feb,emy[i-1], bsc_fde);
    }
    /// feb -> fee : first electron cooling
    else if( tc[i]>tc_feb &&  tc[i]<=tc_fee )   { 
      ke[i]  = stepper(tc_feb,tc_fee,tc_tsv, ke_feb,ke_fee,ke[i-1], bsc_feb);
      emx[i] = stepper(tc_feb,tc_fee,tc_tsv, emx_feb,emx_fee,emx[i-1], bsc_feb);
      emy[i] = stepper(tc_feb,tc_fee,tc_tsv, emy_feb,emy_fee,emy[i-1], bsc_feb);
    }
    /// fee -> sdb : between 1st cooling and 2nd deceleration
    else if( tc[i]>tc_fee &&  tc[i]<=tc_sdb )   { 
      ke[i]  = stepper(tc_fee,tc_sdb,tc_tsv, ke_fee,ke_sdb,ke[i-1], bsc_fee);
      emx[i] = stepper(tc_fee,tc_sdb,tc_tsv, emx_fee,emx_sdb,emx[i-1], bsc_fee);
      emy[i] = stepper(tc_fee,tc_sdb,tc_tsv, emy_fee,emy_sdb,emy[i-1], bsc_fee);
    }
    /// sdb -> sde : 2nd deceleration
    else if( tc[i]>tc_sdb &&  tc[i]<=tc_sde )   { 
      ke[i]  = stepper(tc_sdb,tc_sde,tc_tsv, ke_sdb,ke_sde,ke[i-1], bsc_sdb);
      emx[i] = stepper(tc_sdb,tc_sde,tc_tsv, emx_sdb,emx_sde,emx[i-1], bsc_sdb);
      emy[i] = stepper(tc_sdb,tc_sde,tc_tsv, emy_sdb,emy_sde,emy[i-1], bsc_sdb);
    }
    /// sde -> seb : between 2nd second deceleration and 2nd electron cooling
    else if( tc[i]>tc_sde &&  tc[i]<=tc_seb )   { 
      ke[i]  = stepper(tc_sde,tc_seb,tc_tsv, ke_sde,ke_seb,ke[i-1], bsc_sde);
      emx[i] = stepper(tc_sde,tc_seb,tc_tsv, emx_sde,emx_seb,emx[i-1], bsc_sde);
      emy[i] = stepper(tc_sde,tc_seb,tc_tsv, emy_sde,emy_seb,emy[i-1], bsc_sde);
    }
    /// seb -> see : 2nd electron cooling
    else if( tc[i]>tc_seb &&  tc[i]<=tc_see )   { 
      ke[i]  = stepper(tc_seb,tc_see,tc_tsv, ke_seb,ke_see,ke[i-1], bsc_seb);
      emx[i] = stepper(tc_seb,tc_see,tc_tsv, emx_seb,emx_see,emx[i-1], bsc_seb);
      emy[i] = stepper(tc_seb,tc_see,tc_tsv, emy_seb,emy_see,emy[i-1], bsc_seb);
    }
    /// see -> ext : between 2nd electron cooling and extraction
    /// coasting beam to 4 bunches!
    else if( tc[i]>tc_see &&  tc[i]<=tc_ext )   { 
      ke[i]  = stepper(tc_see,tc_ext,tc_tsv, ke_see,ke_ext,ke[i-1], bsc_see);
      emx[i] = stepper(tc_see,tc_ext,tc_tsv, emx_see,emx_ext,emx[i-1], bsc_see);
      emy[i] = stepper(tc_see,tc_ext,tc_tsv, emy_see,emy_ext,emy[i-1], bsc_see);
    }
    /// else (debug)
    else {
      ke[i]   = -1.*RUnits::keV;
  ...
  cout << setw(10)  << "emy (um)" << "   ";
  cout << endl;
  for( int i=0; i<fc_nsp; ++i ) { 
    cout << setw(3)   << i                  << "   ";
    cout << setw(10)  << tc[i]/RUnits::s    << "   ";
    cout << setw(10)  << ke[i]/RUnits::keV  << "   ";
    cout << setw(10)  << emx[i]/RUnits::um  << "   ";
    cout << setw(10)  << emy[i]/RUnits::um  << "   ";
    cout << endl; 
  }
  ...
  double* tci = &tc[0];
  double* kei = &ke[0];
  TGraph* keg = new TGraph(fc_nsp, tci, kei);
  keg->Draw("ALP");
  gPad->Print("keg.pdf", "pdf");
