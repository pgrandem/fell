...
void elenaCycle()
{
  ...
  const int fc_nsp(51);     /// function const : number of sampling points
  /// time stamps
  double tc_inj(0.*RUnits::s);  /// time cycle - injection;
  double tc_fdb(1.*RUnits::s);  /// time cycle - first deceleration begin
  double tc_fde(5.*RUnits::s);  /// time cycle - first deceleration end
  double tc_feb(6.*RUnits::s);  /// time cycle - first electron cooling start
  double tc_fee(16.*RUnits::s); /// time cycle - first electron cooling end
  double tc_sdb(17.*RUnits::s); /// time cycle - second deceleration begin
  double tc_sde(21.*RUnits::s); /// time cycle - second deceleration end
  double tc_seb(22.*RUnits::s); /// time cycle - second electron cooling start
  double tc_see(25.*RUnits::s); /// time cycle - second electron cooling end
  double tc_ext(26.);           /// time cycle - extraction;
  /// energy stamp
  double ke_inj(5.3*RUnits::MeV);   /// kinetic energy - injection
  double ke_fep(643.*RUnits::keV);  /// kinetic energy - first energy plateau
  double ke_sep(100.*RUnits::keV);  /// kinetic energy - second energy plateau
  /// beam parameters
  double be_ncp=2e7;  /// beam - number of circulationg pbar
  
  /// time array
  /// ----------
  double tc_asp[fc_nsp];  /// time cycle - array of sampling point
  double tc_tsv((tc_ext-tc_inj)/(fc_nsp-1));  /// time cycle - time step value
  for( int i=0; i<fc_nsp; ++i ) { tc_asp[i]=tc_tsv * i + tc_inj; }
  
  /// energy array
  /// ------------
  double ke_asp[fc_nsp];  /// kinetic energy - array of sampling point
  for( int i=0; i<fc_nsp; ++i ) {
    if( tc_asp[i]<tc_fdb ) { ke_asp[i]=ke_inj; }          /// injection
    else if( tc_asp[i]>=tc_fdb &&  tc_asp[i]<tc_fde )   { /// deceleration 1
      double ramp=(ke_fep-ke_inj)/(tc_fde-tc_fdb);
      if( i>0 ) { ke_asp[i]=ke_asp[i-1]+ramp*tc_tsv; }
      else      { ke_asp[i]=ke_inj+ramp*tc_tsv; }       } 
    else if( tc_asp[i]>=tc_fde &&  tc_asp[i]<tc_sdb )   { /// cooling 1
      ke_asp[i]=ke_fep;                                 }
    else if( tc_asp[i]>=tc_sdb &&  tc_asp[i]<tc_sde )   { /// deceleration 2
      double ramp=(ke_sep-ke_fep)/(tc_sde-tc_sdb);
      if( i>0 ) { ke_asp[i]=ke_asp[i-1]+ramp*tc_tsv; }
      else      { ke_asp[i]=ke_fep+ramp*tc_tsv; }       } 
    else ke_asp[i]=ke_sep;
  }
  
  /// emittance array
  double emx_asp[fc_nsp]; /// horizontal physical emittance (um, sigma2/TwissB)
  double emy_asp[fc_nsp]; /// vertical physical emittance (um, sigma2/TwissB)
  double emz_asp[fc_nsp]; /// longitudinal emittance 4pi sigmaE sigmaT
  
  
  
  /// same thing with interpol function
  double tint(0);
  ...
  cout << endl;
  for( int i=0; i<fc_nsp; ++i ) { 
    tint = RMath::interpol(tc_inj, tc_ext, fc_nsp, i, "linear");
    
    cout << setw(2)   << i << "   ";
    cout << setw(5)   << tc_asp[i] << "   ";
    cout << setw(10)  << ke_asp[i]/RUnits::keV << "   ";
    cout << endl; 
  }
